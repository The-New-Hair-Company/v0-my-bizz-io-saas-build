...
export async function POST(request: Request) {
  ...
  let entitlement
  try {
    entitlement = await consumeEntitlement(organizationId, 'grounded_chat')
  } catch (error) {
  ...
  if (!entitlement.allowed) {
    return Response.json({
      error: 'Your monthly grounded chat allowance has been used.',
      code: 'PLAN_LIMIT_REACHED',
      usage: entitlement,
      upgradeUrl: '/pricing?source=chat-limit',
    }, { status: 402 })
  }
