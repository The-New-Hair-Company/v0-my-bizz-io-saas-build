...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase.rpc('search_grounded_knowledge', {
    query_text: buildRetrievalQuery(parsed.data.message, 'public'),
    p_organization_id: null,
    match_count: 4,
    p_categories: ['platform', 'security', 'ai', 'delivery'],
  })
  ...
  return Response.json({
    answer: composeGroundedAnswer(parsed.data.message, sources, 'public'),
    sources: sources.map((source) => ({ id: source.source_id, title: source.title, score: source.score, url: source.source_url })),
  })
}
